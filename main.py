...
def main():
    ...
    highscores_button = Button(SCREEN_WIDTH // 2 - 290, SCREEN_HEIGHT // 2 + 40, 540, 70, "NEJLEPŠÍ VÝSLEDKY", GAME_STATE_HIGHSCORES) 
        ...
        if current_game_state == GAME_STATE_INTRO:
            ...
            if player.lives <= 0 or remaining_time_seconds <= 0 or pygame.sprite.spritecollideany(player, exits):
                if current_game_state == GAME_STATE_PLAYING:
                    current_game_state = GAME_STATE_GAME_OVER
                    if pygame.sprite.spritecollideany(player, exits) and remaining_time_seconds > 0 and player.lives > 0:
            ...
            screen.blit(lives_text_surface, (SCREEN_WIDTH // 2 - lives_text_surface.get_width() // 2, (HUD_HEIGHT - lives_text_surface.get_height()) // 2))

            charges_text_surface = hud_font.render(f"Nálože: {player.demolition_charges}", True, WHITE)
            screen.blit(charges_text_surface, (SCREEN_WIDTH - charges_text_surface.get_width() - 10, (HUD_HEIGHT - charges_text_surface.get_height()) // 2))

            timer_text_surface = hud_font.render(f"Čas: {max(0, remaining_time_seconds)}s", True, WHITE)
            screen.blit(timer_text_surface, (SCREEN_WIDTH - charges_text_surface.get_width() - 10 - timer_text_surface.get_width() - 20, (HUD_HEIGHT - timer_text_surface.get_height()) // 2))
